...
- [ ] 빈 상태(empty)가 처리되었고 `EmptyState` 컴포넌트/동등 UI로 다음 행동을 안내하는가?
...
- [ ] project/config.md §Mandatory-Search-Paths 공통 컴포넌트으로 대체 가능하지 않은가?
- [ ] 동일/유사 컴포넌트이 여러 feature에 별도 구현되지 않았는가?
...
- [ ] 사용처 1개에 옵션 파라미터 5개 이상의 컴포넌트이 없는가?
- [ ] "나중을 위해" 외 다른 정당화가 없는 코드가 없는가?

#### 체크리스트 2.4 — 단일 책임 검사 [P4]
- [ ] 50줄 초과 `render()` 메서드가 분리 검토되었는가?
- [ ] Component이 UI 렌더링 + 비즈니스 로직 + 상태 관리를 한꺼번에 하지 않는가?
- [ ] 함수/컴포넌트 이름에 `and`/`그리고` 가 없는가?
...
#### 체크리스트 3.1 — 변경 파급 분석
- [ ] 변경된 공통 컴포넌트의 모든 사용처 점검했는가? (project/config.md §Architecture 공통 컴포넌트 경로)
...
- [ ] 권한 검증이 UI뿐 아니라 Hook/Context/Service에서도 이루어지는가?
