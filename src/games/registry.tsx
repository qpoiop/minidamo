      ...
      sections: [
        ...
        },
        {
          ...
          items: [
            ...
            { label: '조커 · [멈춤]으로 넣을 때만 소유자가 위치 직접 선택', tone: 'accent' },
          ],
        },
        {
          title: '핵심 감각 · 배지',
          ...
          items: [
            ...
            { label: '시작 손패 · 오답 강제공개 조커는 자동으로 맨 뒤', tone: 'muted' },
          ],
        },
      ],
      steps: [
        ...
        { title: '정렬 규칙', desc: '값 오름차순. 동수 색상 우선: 검정 < 흰색. 조커는 정답 후 [멈춤]으로 넣을 때만 소유자가 위치를 직접 고를 수 있고, 시작 손패에 있거나 오답으로 강제 공개될 땐 선택 없이 자동으로 맨 뒤에 놓임. 새 타일은 규칙 자리에 자동 삽입되며 위치 자체가 상대에게 힌트가 됨.' },
      ...
      sections: [
        {
          ...
          items: [
            ...
            { label: '레이즈 +N', desc: '슬라이더로 증액 N 선택 후 [레이즈] 탭. currentBet 갱신 · 상대 응대 대기. 레이즈 5회째엔 자동으로 강제 쇼다운.', glyph: 'target', tone: 'accent' },
        ...
        },
        {
          ...
          items: [
            ...
            { label: '쇼다운', desc: '양 카드 공개 · 높은 값이 팟 획득. 동률은 팟 분할 (홀수 칩은 그 판을 매듭지은 쪽 — 마지막 콜/체크한 사람 — 에게 · 레이즈 상한으로 강제 종료됐다면 마지막 레이즈한 사람의 상대에게).', glyph: 'check', tone: 'accent' },
          ],
        },
        {
          title: '핵심 감각 · 배지',
          kind: 'badges',
          items: [
            { label: '카드는 매 판 독립 추첨 · 상대 카드 값 자체로 내 카드 예측 불가', tone: 'muted' },
            { label: '상대가 세게 나오면 내 카드가 낮다는 신호', tone: 'muted' },
            { label: '블러핑 균형 · 항상 정직 = 읽힘', tone: 'accent' },
            { label: '베이지안 추론 · 상대의 베팅 강도로 내 카드 추정', tone: 'accent' },
            { label: '내 칩 스택 상시 확인 · 파산 위험 관리', tone: 'bomb' },
          ],
        },
      ],
      steps: [
        { title: '개요', desc: '2인 심리 베팅. 매 판 1장씩 · 자기 것은 못 보고 상대 것만 봄. 상대의 베팅 반응을 읽어 내 카드가 이길 확률을 역추론.' },
        { title: '카드 · 옵션', desc: '공유 덱 없이 매 판 각자 1~10 중 무작위 1장을 독립 추첨(중복 가능) — 이전 판·상대 카드와 무관. 시작 칩 20/30/50 · 앤티 1. 매치 판 수 9/15/21 선택 후 종료 시 칩 많은 쪽 승 · 상대 칩 0 시 즉시 승.' },
        { title: '진행 방식', desc: '앤티 → 배분 → 선공부터 순차 베팅 → 콜 맞추면 쇼다운 → 팟 이동 → 다음 판 선공 교대. 폴드 시 팟 상대에게.' },
        { title: '베팅 상한', desc: '레이즈는 5회째에 자동으로 강제 쇼다운(그 이상 주고받기 불가). 남은 칩 초과 시 자동 all-in 스냅.' },
        { title: '심리 추론', desc: '카드는 매 판 독립 추첨이라 상대 카드 값 자체는 내 카드와 무관 — 대신 상대는 내 카드를 보고 베팅하므로 그 반응이 단서가 됨. 상대가 안 죽고 세게 나오면 → "내 카드가 낮다는 신호", 상대가 소극적이면 → 내 카드가 높아 조심한다는 신호. 이 행동 신호를 읽어 콜/폴드 결정.' },
        { title: '블러핑 균형', desc: '항상 정직하게 베팅하면 읽힘. 낮은 상대에 크게 질러 폴드 유도하는 세미블러프를 가끔 섞어 상대 콜 성향을 무너뜨리기.' },
        { title: '동률 처리', desc: '팟 분할 (홀수 칩은 그 판을 매듭지은 쪽 — 마지막 콜/체크한 사람 — 에게 · 레이즈 상한으로 강제 종료됐다면 마지막 레이즈한 사람의 상대에게). 옵션으로 "무효·재배분" 도 검토 가능 (현 사이클은 분할 고정).' },
        { title: '승리 조건', desc: '정해진 판 수 후 칩 많은 쪽 승 (칩까지 동률이면 매치 무승부). 도중 상대 칩 0 시 즉시 승 (파산 즉시 종료).' },
      ],
      warning: {
  ...
  },
  {
    ...
    guide: {
      ...
      steps: [
        ...
        { title: '종반 (더미 소진)', desc: '더미 마르면 보충 없이 손패 3장으로 마지막 3트릭 마무리(40장 ÷ 2장 = 20트릭 중 나머지). 이때부터 무늬 따르기 강제: 후는 리드 무늬 있으면 반드시 따름 · 없으면 트럼프 우선 · 그도 없으면 아무 카드. 리드는 이 국면에도 계속 아무 카드나 자유 선택(강제는 후에게만 적용). 낼 수 없는 카드는 흐리게 잠금.' },
